		...
		for (int y = pos.y; y < pos.y+height; y++) {
			for (int x= pos.x; x < pos.x+width+1; x++) {
				for (int z = pos.z; z < pos.z+depth+1; z++) {
					...
					NBTTagCompound currentBlockCompound = (NBTTagCompound) compound.getTag("Block|"+relativeX+"|"+relativeY+"|"+relativeZ);
					if(currentBlockCompound==null){
						Minecraft.getMinecraft().crashed(new CrashReport("compound is null", new NullPointerException()));
					}
					NBTTagCompound tileEntityCompound=currentBlockCompound.getCompoundTag("tileEntityTag");
					int metadata=currentBlockCompound.getInteger("metadata");
					
					Block currentBlock=Block.getBlockFromName(currentBlockCompound.getString("blockid"));
